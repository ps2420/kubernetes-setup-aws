...
--set server.service.type=ClusterIP --set server.persistence.size=50Gi \
--set server.resources.requests.memory=1024Mi --set server.resources.limits.memory=1024Mi \
--set agent.resources.limits.memory=1024Mi --set agent.resources.requests.memory=1024Mi -f ./$PREFIX_DIR/values.yaml
...
--set agent.security.ssh.enabled=true --set server.persistence.size=50Gi

#Delete existing ingress and create new one
sed -e "s/GOCD_DNS/$GOCD_DNS/g" $PREFIX_DIR/ingress-template.yaml | kubectl apply -f -
